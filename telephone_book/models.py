...
# import requests
import arrow  # interface zum einfachen Verarbeiten von Zeitzonen....
...
class Entry:
    ...
    def phone_number(self, number):
        """Validatoren u.U. auslagern in eigene Funktionen/Klassen"""
        # if len(number) < 5:
        #    raise ValidationError(
...
class TelephoneBook:
    ...
    def __add__(self, entry: Entry):
        """Telephonebook + Eintrag fügt einen neuen Eintrag 
        in das Telefonbuch"""
...
if __name__ == '__main__':
    ...
    my_book = TelephoneBook("mein privates Telefonbuch",
                            Config.config())
    # my_book.add(dieter)

    # Entry Objekt Donald via + Operator zum Telefonbuch
    my_book = my_book + donald + dieter

    # Donald wieder aus Telefonbuch löschen mit - Operator
    my_book = my_book - donald

    my_book.add(klaus)
    # len(my_book)
    # it = iter(my_book)
    # print("ITER my_book:", it)
